...
impl<'a> Lexer<'a> {
    ...
    pub(crate) fn tokens<'l>(&'l mut self) -> Tokens<'l, 'a> {
        Tokens { lexer: self }
    }
                ...
                if self.input.accept('=') {
                    ...
                    self.add_diagnostic(
                        Diagnostic::error(self.file_id, "expected declaration")
                            .primary_span(self.input.span_after(start)),
                    );
    ...
    fn lex_end_of_action(&mut self) -> Token {
        ...
        if has_trim_marker && has_right_delim {
            self.add_diagnostic(
                ...
                .primary(
                    self.input.span_after(start_pos),
    ...
    fn lex_comment(&mut self) -> Token {
        let start_span = self.input.must_consume(LEFT_COMMENT_MARKER);
        ...
        if self.input.accept(RIGHT_COMMENT_MARKER) {
            ...
            if !self.at_end_of_action() {
                self.add_diagnostic(
                    Diagnostic::error(self.file_id, "comment ends before closing delimiter")
                        .primary(self.input.zero_width_span(), "...and ends here")
            ...
            self.add_diagnostic(
                Diagnostic::error(self.file_id, "unclosed comment")
                    .primary(self.input.span(), "...but the file ends here")
                    .secondary(start_span, "a comment starts here")
    ...
    fn lex_string(&mut self) -> Token {
        let start_span = self.input.must_consume('"');
        ...
        if !self.input.accept('\'') {
            self.add_diagnostic(
                Diagnostic::error(self.file_id, "unterminated character literal")
                    .primary(self.input.zero_width_span(), "...and should end here")
                    .secondary(start_span, "a character literal starts here")
    ...
    fn read_char(&mut self, ctx: ReadCharContext, open_delim_span: Span) -> Result<char, ()> {
            ...
            if first == '\n' {
                let mut err =
                    Diagnostic::error(self.file_id, format!("unexpected newline in {ctx}"))
                        ...
                        .secondary(open_delim_span, format!("a {ctx} starts here"));
                if ctx == ReadCharContext::QuotedString {
                    err = err.footer_note(indoc! {r#"
                        help: to create a newline in a quoted string, use the escape sequence `\n`
        ...
        let escapee = self.input.next().ok_or_else(|| {
            self.add_diagnostic(
            ...
            Ordering::Less => {
                self.add_diagnostic(
                    Diagnostic::error(self.file_id, "too few digits in escape sequence")
                        .primary_span(self.input.span_after(start_pos)),
                );
                Err(())
            }
            Ordering::Equal => {
    ...
    fn lex_numeric_literal(&mut self) -> Token {
        ...
        let span = self.input.span_after(start_pos);
        let text = &self.input.text[span.range()];
    ...
    fn mk_tok(&mut self, kind: TokenKind) -> Token {
        let token = Token::new(kind, self.input.span_after(self.last_token_end));
...
mod tests {
    ...
    fn text_with_comment() {
        assert_lex_ok(
            ...
            vec![
                ...
                tok(LeftActionDelim(false), span(6..8)),
                tok(Comment(" this is a comment ".to_string()), span(8..31)),
                tok(RightActionDelim(false), span(31..33)),
    ...
    fn empty_action() {
        assert_lex_ok(
            "{{}}",
            vec![
                tok(LeftActionDelim(false), span(0..2)),
                tok(RightActionDelim(false), span(2..4)),
            ],
        );
    }
    ...
    fn identifier() {
        assert_lex_ok(
            ...
            vec![
                ...
                tok(RightActionDelim(false), span(5..7)),
            ],
        );
    }
    ...
    fn parens() {
        assert_lex_ok(
            ...
            vec![
                ...
                tok(RightActionDelim(false), span(13..15)),
            ],
        );
    }
    ...
    fn block() {
        assert_lex_ok(
            ...
            vec![
                ...
                tok(Dot, span(14..15)),
                tok(RightActionDelim(false), span(15..17)),
            ],
        );
    }
    ...
    fn raw_string_with_newline() {
        ...
        assert_lex_ok(
            ...
            vec![
                ...
                tok(RightActionDelim(false), span(24..26)),
            ],
        );
    }
